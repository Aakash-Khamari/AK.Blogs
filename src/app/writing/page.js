...
import { ArrowRight, Loader2, BookOpen, PenTool, Lightbulb, Activity, Zap, Search } from 'lucide-react'
...
export default function WritingIndex() {
  ...
  const getIconForCategory = (category) => {
    switch (category?.toLowerCase()) {
      ...
      case 'design': return <PenTool size={18} className="text-white" />
      case 'technology': return <Search size={18} className="text-white" />
      case 'psychology': return <Lightbulb size={18} className="text-white" />
  ...
  const getColorForCategory = (category) => {
    switch (category?.toLowerCase()) {
      ...
      case 'psychology': return 'from-green-400 to-emerald-600'
  ...
  return (
    <main className="min-h-screen relative overflow-hidden transition-colors duration-500">
      {/* Animated Mesh Gradient Background (Subdued for inner pages) */}
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none opacity-20 dark:opacity-10 mix-blend-multiply dark:mix-blend-screen">
        <motion.div
          ...
          className="absolute top-0 right-0 w-[40%] h-[40%] bg-purple-200 dark:bg-purple-900 rounded-full blur-[120px]"
        />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pb-32 pt-24">
        <div className="mb-20">
          ...
          <p className="text-xl text-neutral-500 dark:text-neutral-400 font-sans max-w-2xl leading-relaxed">
            Observations, unfinished thoughts, and deeper explorations of human behavior and technology.
          </p>
        </div>
          ...
          <div className="space-y-16">
            {loading ? (
              ...
              <div className="text-center py-20 text-neutral-500 font-medium pl-24">
                No writing recorded yet.
              </div>
            ) : (
              posts.map((post, index) => {
                const color = getColorForCategory(post.category)
                ...
                return (
                  ...
                  >
                    <div className={`absolute left-2 md:left-6 top-1/2 -translate-y-1/2 w-5 h-5 bg-gradient-to-br ${color} rounded-full z-10 shadow-lg ring-4 ring-white dark:ring-[#0a0a0a]`} />
                    ...
                    <Link href={post.type === 'observation' ? `/observations/${post.slug}` : `/notebook/${post.slug}`}>
                      <div className="bg-white dark:bg-[#111] p-6 md:p-10 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] dark:shadow-[0_10px_40px_-10px_rgba(255,255,255,0.03)] border border-neutral-100 dark:border-neutral-800 group hover:-translate-y-2 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_20px_60px_-15px_rgba(255,255,255,0.05)] transition-all duration-300 cursor-pointer relative overflow-hidden">
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 relative z-10">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-black text-black dark:text-white uppercase tracking-widest">
                              {post.category || (post.type === 'observation' ? 'Observation' : 'Notebook')}
                            </span>
                          </div>
                        ...
                        <p className="text-base text-neutral-600 dark:text-neutral-400 leading-relaxed line-clamp-3 font-sans relative z-10">
                          {post.content_story || post.content || "Explore this thought further..."}
                        </p>
